...
namespace ogdf{
	...
	void Triconnectivity::DFS1(const Graph& G, node root, node dummy){
		int n = G.numberOfNodes();

		node* stack = new node[n];
		adjEntry* temp_adj = new adjEntry[n];

		m_NUMBER[root] = ++m_numCount;
		m_FATHER[root] = nullptr;
		m_DEGREE[root] = root->degree();

		m_LOWPT1[root] = m_LOWPT2[root] = m_NUMBER[root];
		m_ND[root] = 1;

		stack[0]=root;
		temp_adj[0]=root->firstAdj();
		int SP=0;
		while(SP!=-1){
			char down=0;
			node v = stack[SP];
			for(adjEntry adj = temp_adj[SP]; adj!=nullptr; adj=adj->succ()){
				edge e = adj->theEdge();
				...
				if(m_NUMBER[w] == 0){
					...
					//DFS1(G,w,v);
					m_NUMBER[w] = ++m_numCount;
					m_FATHER[w] = v;
					m_DEGREE[w] = w->degree();

					m_LOWPT1[w] = m_LOWPT2[w] = m_NUMBER[w];
					m_ND[w] = 1;
					stack[SP+1]=w; temp_adj[SP+1]=w->firstAdj(); temp_adj[SP]=adj;
					down=1; break;
				}
				//if(v==root){continue;}
				if(v==m_FATHER[w]){
				...
				} else if(m_NUMBER[w]<m_NUMBER[v] && w!=m_FATHER[v]){
	...
	void Triconnectivity::pathFinder(const Graph& G, node start){
		int n=G.numberOfNodes();
		node* stack = new node[n];
		ListIterator<edge>* temp_edge = new ListIterator<edge>[n];
		char* first_time = new char[n];

		stack[0]=start;
		temp_edge[0]=m_A[start].begin();
		first_time[0]=1;

		int SP=0;
		while(SP!=-1){
			node v = stack[SP];
			if(first_time[SP]){ m_NEWNUM[v] = m_numCount - m_ND[v] + 1; first_time[SP]=0; } else{ m_numCount--; }

			char down=0;
			for(ListIterator<edge> e = temp_edge[SP]; e.valid(); e=e.succ()){
				node w = (*e)->opposite(v);
				...
				if(m_TYPE[*e] == EdgeType::tree){
					...
					stack[SP+1]=w; temp_edge[SP+1]=m_A[w].begin(); first_time[SP+1]=1;
					temp_edge[SP]=e.succ();
					down=1; break;
	...
	/*void Triconnectivity::pathSearch (const Graph& G, node v)
	{
		...
		ListIterator<edge> it, itNext;
		for(it = m_A[v].begin(); it.valid(); it=itNext)
		{
			itNext = it.succ();
			e = *it;
			node w = e->target();
	...
	void Triconnectivity::pathSearch(const Graph& G, node start){
		int n = G.numberOfNodes();

		node* stack = new node[n];
		ListIterator<edge>* temp_edge = new ListIterator<edge>[n];
		ListIterator<edge>* temp_next = new ListIterator<edge>[n];
		char* first_time = new char[n];
		edge* stored_edge = new edge[n];

		int* temp_outv = new int[n];
		int* temp_y = new int[n];
		int* temp_a = new int[n];
		int* temp_b = new int[n];
		int* temp_vnum = new int[n];
		int* temp_wnum = new int[n];

		stack[0]=start;
		temp_edge[0]=m_A[start].begin();
		temp_outv[0]=m_A[start].size();
		first_time[0]=1;
		...
		while(SP!=-1){
			...
			if(!first_time[SP]){
				it = temp_edge[SP];
				y=temp_y[SP]; a=temp_a[SP]; b=temp_b[SP]; vnum=temp_vnum[SP]; wnum=temp_wnum[SP];
				...
				if(m_LOWPT2[w] >= vnum && m_LOWPT1[w] < vnum && (m_FATHER[v] != m_start || temp_outv[SP] >= 2)){
#ifdef TRIC_COMP_OUTPUT
					std::cout << "\nfound type-1 separation pair " <<
						m_pGC->original(m_NODEAT[m_LOWPT1[w]]) << ", " <<
						m_pGC->original(v);
				...
				temp_edge[SP]=temp_next[SP];
			}

			first_time[SP]=1;
			for(it = temp_edge[SP]; it.valid(); it=itNext){
				itNext = it.succ();
				e = *it;
				node w = e->target();
				int wnum = m_NEWNUM[w];
				...
				if(m_TYPE[e] == EdgeType::tree){
					...
					//pathSearch(G,w);
					stack[SP+1]=w; temp_edge[SP+1]=m_A[w].begin(); temp_outv[SP+1]=m_A[w].size(); first_time[SP+1]=1;
					temp_edge[SP]=it; temp_next[SP]=itNext; first_time[SP]=0;
					temp_y[SP]=y; temp_a[SP]=a; temp_b[SP]=b; temp_vnum[SP]=vnum; temp_wnum[SP]=wnum; stored_edge[SP]=e;
					descend=1; break;
				} else{ // frond arc
					if(m_START[e]){
						y = 0;
...
int main(int argc, char* argv[]){
   ...
   FILE* fp = fopen(argv[1],"r");
	...
	if(!GraphIO::read(G, argv[1], GraphIO::readGML)){
